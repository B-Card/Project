...
public class Register extends AppCompatActivity {
    ...
    CheckBox driver,passenger,admin;
    TextView login;
    boolean valid = true;
    Button create;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        passenger = findViewById ( R.id.isPassengerId);
            ...
            if(valid)
            {
                fAuth.createUserWithEmailAndPassword(email.getText().toString(),password.getText().toString()).addOnSuccessListener(authResult -> {
                    ...
                    if(passenger.isChecked ()){
                        ...
                        userInfo.put ( "isPassengerId","1" );

                        Map<String,Object> passenger = new HashMap<>();
                        passenger.put("FullName",name.getText().toString());
                        passenger.put("UserEmail",email.getText().toString());
                        passenger.put("PhoneNumber",phone.getText().toString());
                        passenger.put("uid",uid);
                        passenger.put ( "type", "passenger");
                        FirebaseDatabase database=FirebaseDatabase.getInstance();
                        DatabaseReference reference=database.getReference("passenger");

                        reference.child(uid).setValue(passenger);
                    }
